...
class Mobility:
    ...
    def extract(cls, board: chess.Board) -> 'Mobility':
        ...
        for color, key in zip(colors, keys):
            ...
            # FIX 2: Safely get legal moves without corrupting board validity if opponent is in check
            legal_moves = []
            if board.turn == color:
                legal_moves = list(board.legal_moves)
            else:
                # Use pseudo-legal moves for the opponent to avoid invalid state crashes
            ...
            for move in legal_moves:
                ...
                # FIX 3: Ensure we only evaluate mobility for major/minor pieces to keep scoring consistent
                if from_sq not in piece_safe_destinations:
                    continue
                ...
                for atk_sq in enemy_attackers:
                    ...
                    if atk_piece:
                        # FIX 1: Provide a fallback high value (100) for the King so it doesn't trigger 0 < piece_val
                        atk_val = PIECE_VALUES.get(atk_piece.piece_type, 100)
